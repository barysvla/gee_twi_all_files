...
def compute_flow_accumulation_quinn_1991(
    flow_weights,
    *,
    ...
    """
    Topological flow accumulation given precomputed outflow weights.
    - Works for Quinn (1991) MFD/FD8 weights or any (H,W,8) weight tensor.
    - 'cells' accumulates unit contributions; 'm2'/'km2' accumulates area.
    - 'sca' returns Specific Catchment Area = contributing area [m^2] / contour length [m].

    Notes:
    - Per-cell weights are optionally renormalized to sum to 1 over downslope neighbors.
    - Requires a direction raster with no cycles (flats/sinks must be resolved beforehand).
    """
    ...
    # Clamp negatives and mask NoData
    ...
    # Initialize accumulation in float64 for stability
    if out in ('cells', 'sca'):
    ...
    # In-degree for Kahn topological processing
    ...
    if out == 'km2':
        ...
        acc = acc / dx[:, None]  # SCA = area [m^2] / contour length [m]
